    ...
    describe("The Context Graph Modeller:", function () {
        ...
        describe("Given a microservice with no dependencies in a particular context", function () {
            ...
            describe("when I apply the transform function", function () {
                ...
                it("then the node should have an ID matching its context", function () {
                    expect(result.id).toBe(contextNames[0]);
                });
                ...
                it("then the label function should have the correct context name", function () {
                    expect(result.label).toBe(contextNames[0]);
                });

                it("then the hover text should contain the context name", function () {
                    expect(result.customHover).toContain(contextNames[0]);
                });
        ...
        describe("Given two microservices from a different context", function () {
            ...
            describe("when I extract the edge between the two", function () {
                var result = contextGraphModeller.extractEdge(source, target)
                ...
                it("then the source of the edge should be the source node's context", function () {
                    expect(result.source).toBe(contextNames[0]);
                });

                it("then the target of the edge should be the target node's context", function () {
                    expect(result.target).toBe(contextNames[1]);
                })
